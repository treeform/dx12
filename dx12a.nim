...
proc releaseCom(obj: pointer) =
  if obj != nil:
    type ReleaseProc = proc(this: pointer): uint32 {.stdcall.}
    discard callVtbl0(obj, 2, ReleaseProc)
...
proc SetEventOnCompletion(self: ID3D12Fence, value: UINT64, evt: winlean.Handle): HRESULT =
  type F = proc(this: ID3D12Fence, value: UINT64, evt: winlean.Handle): HRESULT {.stdcall.}
...
proc cleanup(ctx: var D3D12Context) =
  ctx.waitForGpu()
  for i in 0..<FRAME_COUNT:
    releaseCom(ctx.renderTargets[i])
  releaseCom(ctx.commandAllocator)
  releaseCom(ctx.commandList)
  releaseCom(ctx.commandQueue)
  releaseCom(ctx.descriptorHeap)
  releaseCom(ctx.swapChain)
  releaseCom(ctx.device)
  releaseCom(ctx.fence)
  if ctx.fenceEvent != 0:
